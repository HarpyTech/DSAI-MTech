...
select 
	trim(substring_index(Player, '(', 1)) AS name,
    substring_index(substring_index(Player, '(', - 1),')', 1) AS country
from
    test_batting;
-- 2913 Records Returned    
alter table test_batting 
	add column player_name varchar(50)  generated always as (trim(substring_index(Player, '(', 1))) stored,
    add column country varchar(15)  generated always as (substring_index(substring_index(Player, '(', - 1),')', 1)) stored;
...
select 
	span,
	substring_index(span, '-', 1) AS start_year,
    substring_index(span, '-', - 1) AS end_year
from
    test_batting;
-- 2913 Records Returned    
alter table test_batting 
...
select no, hs, substring_index(hs, "*", 1) as highest_runs,
case hs
when hs like "%*" then "Not Out"
else "Out"
end as not_out_status from test_batting;
-- 2913 Records Returned    
alter table test_batting 
...
select * from test_batting where 2019 between start_year and end_year order by avg desc limit 6;
...
select * from test_batting where 2019 between start_year and end_year order by `100` desc limit 6;
...
select * from test_batting 
	where (2019 between start_year and end_year) and inn > 100
    order by highest_runs desc limit 6;
...
create view Batting_Order_GoodAvgScorers_SA as select * from test_batting where 2019 between start_year and end_year order by `100` desc limit 6;
...
create view Batting_Order_HighestCenturyScorers_SA as select * from test_batting where 2019 between start_year and end_year order by `100` desc limit 6;

# 11.	Using the data given, Give the number of player_played for each country.
select count(distinct player_name) as players_played, country from test_batting group by country;
-- Rows count: 27
