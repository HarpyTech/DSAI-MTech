...
select first_name, ntile(4) over (
        partition by
            department_id
    )
from employees
...
select salary, department_id, first_value(first_name) over (
        partition by
            department_id
        order by salary desc
    )
...
select distinct
    ...
    last_value(start_date) over (
        partition by
            j.employee_id
        order by
            start_date range between unbounded preceding
...
-- --------------------------------------------------------------------
-- nth_value
select department_id.first_name, salary, nth_value(first_name, 2) over (
        partition by
            department_id
        order by salary desc
    ) as `2nd_highest`
...
select
    ...
    sum(salary) over (
        partition by
            department_id rows between unbounded preceding
            and current row
    ) as t11_salary,
    sum(salary) over (
        partition by
            department_id rows between 1 preceding
            and 1 following
    ) as `11_salary`,
    sum(salary) over (
        partition by
    ...
    sum(salary) over (
        partition by
            department_id rows between 2 preceding
            and 3 following
    ) as `23_salary`,
    salary
from employees
...
select
    ...
    sum(salary) over (
        partition by
            department_id range between unbounded preceding
            and current row
    ) as t11_salary,
    sum(salary) over (
        partition by
            department_id
        order by salary range between 200 preceding
            and 200 following
    ) as `11_salary`,
    salary
from employees
...
select min(avg_sal), max(avg_sal)
from minmax;
# Find the big and small departments wrt to the count of employees
with
    count1 as (
        select department_id, count(*) as c_emp
        from employees
        group by
            department_id
    )
select min(c_emp), max(c_emp)
from count1;
-- Find percentile rank  of every dept by total salary.
with
    perc_rank as (
        select department_id, sum(salary) as t_sal
        from employees
        group by
            department_id
    )
select t_sal, percent_rank() over (
        order by t_sal
    )
from perc_rank;
...
explain format = JSON
select count(*) as emp_count, sum(salary) as total_sal
from employees
where
    department_id = 50;