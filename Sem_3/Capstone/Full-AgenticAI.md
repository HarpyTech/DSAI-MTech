...
@tool
def predict_heart_attack_risk(patient_info: str) -> str:
    """Predict the heart attack risk based on structured patient information."""
    prompt = f"""
    Given the following patient data:
    {patient_info}
    Predict the risk level (Low, Medium, High) of a heart attack and provide brief reasoning.
    """
    model = GenerativeModel("gemini-pro")
    response = model.generate_content(prompt)
    return response.text
...
@tool
def classify_diabetes(patient_metrics: str) -> str:
    """Classify the type of diabetes based on medical attributes."""
    prompt = f"""
    Analyze the following patient data:
    {patient_metrics}
    Predict the type of diabetes (Type 1, Type 2, Prediabetes, NDM, etc.) and explain the rationale.
    """
    model = GenerativeModel("gemini-pro")
    response = model.generate_content(prompt)
    return response.text
...
def identify_task_and_entities(text: str) -> str:
    """Identify whether the input is for treatment summary, heart risk, or diabetes classification."""
    prompt = f"""
...
tools = [
    ...
    Tool(
        ...
        description="Classify diabetes type from patient metrics like insulin, glucose, BMI, genetics."
    ),
    Tool(
        name="identify_task_and_entities",
        func=identify_task_and_entities,
        description="Determine which model should be used based on user input"
    )
]
